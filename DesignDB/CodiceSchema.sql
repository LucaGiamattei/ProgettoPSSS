...
CREATE TABLE IF NOT EXISTS `Prova2`.`Lezione` (
  ...
  `prezzo` FLOAT NULL,
  `Topic_idTopic` INT NOT NULL,
  `DescrizioneLezione` VARCHAR(300) NULL,
...
CREATE TABLE IF NOT EXISTS `Prova2`.`FasciaOraria` (
  ...
  `Visibile` INT NOT NULL DEFAULT 0,
  `Lezione_idLezione` INT NOT NULL,
  `Lezione_Utente_idUtente` INT NOT NULL,
  `idFasciaOraria` INT NULL AUTO_INCREMENT,
...
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `Prova2`.`CatalogoLezioni` (`idLezione` INT, `NomeLezione` INT, `MediaScoreLezione` INT, `NMaxStudenti` INT, `DataLezione` INT, `OrarioLezione` INT, `prezzo` INT, `visibile` INT, `Utente_idUtente` INT, `Topic_idTopic` INT, `idFasciaOraria` INT);
...
USE `Prova2`;
CREATE  OR REPLACE VIEW `CatalogoLezioni` AS
SELECT Lezione.idLezione, Lezione.NomeLezione, Lezione.MediaScoreLezione, Lezione.NMaxStudenti, FasciaOraria.DataLezione, FasciaOraria.OrarioLezione, Lezione.prezzo,FasciaOraria.visibile, Lezione.Utente_idUtente, Lezione.Topic_idTopic, FasciaOraria.idFasciaOraria FROM Lezione INNER JOIN FasciaOraria ON Lezione.idLezione = FasciaOraria.Lezione_idLezione && Lezione.Utente_idUtente = FasciaOraria.Lezione_Utente_idUtente  WHERE (DataLezione >CURDATE() OR ( DataLezione =CURDATE()  &&OrarioLezione>= DATE_FORMAT(CURTIME(), '%H')*2));
